...
- Body page numbering accidentally counting cover/TOC front matter; the body must start in a new section with page numbering restarted at 1, and front matter must not display the body page-number field.
...
- `scripts/build_report.py` now starts the body in a new section after the TOC, removes body page-number fields from front matter footers, and restarts body page numbering at 1, so cover and TOC pages are not counted as body pages.
