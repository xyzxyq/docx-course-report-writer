...
默认封面必须只占第一页；若第二页只剩日期、空白封面残留或封面元数据，需要压缩封面空段并重新导出。目录页码应通过 Word 字段和右制表位统一对齐。所有插图应使用 `图x.x 标题` 形式的正式图注，图片来源、AI 生成状态和 prompt 记录默认放在 `image-attributions.md` 等旁路文件中，不写在图注下方正文里。
